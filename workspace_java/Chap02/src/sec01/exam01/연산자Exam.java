...
//		5000원, 1000원, 500원, 100원, 50원, 10원, 1원
		...
		int countg = sum / w; // 1000원권 개수
		sum = sum % w; // 1000원권을 제외한 나머지 금액

		int counte = sum / e; // 500원권 개수
		sum = sum % e; // 500원권을 제외한 나머지 금액

		int countr = sum / r; // 100원 주화 개수
		sum = sum % r; // 100원 주화를 제외한 나머지 금액

		int countq = sum / q; // 50원 주화 개수
		sum = sum % q; // 50원 주화를 제외한 나머지 금액

		int countt = sum / t; // 10원 주화 개수
		sum = sum % t; // 10원 주화를 제외한 나머지 금액

		int county = sum / y; // 1원 주화 개수
		System.out.println(
			    "필요 금액권 : " + w + "원권 " + countg + "장, "
			    				 + e + "원권 " + counte + "장, "
			    				 + r + "원 주화 " + countr + "개, "
			    				 + q + "원 주화 " + countq + "개, "
			    				 + t + "원 주화 " + countt + "개, "
